    ...
    <div className="pt-32 pb-24 min-h-screen bg-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          >
            <span className="text-[#d4af37] font-bold uppercase tracking-widest text-xs mb-4 block">Our Culinary Collection</span>
            <h1 className="text-5xl md:text-6xl font-serif font-bold text-white mb-6">
              The <span className="gold-gradient">Menu</span>
            </h1>
            <p className="text-white/50 max-w-2xl mx-auto leading-relaxed">
              Explore our diverse range of dishes, from traditional favorites to modern innovations. 
              Each dish is prepared with the freshest ingredients and a touch of VIP luxury.
            </p>
          </motion.div>
        </div>
        ...
        <div className="space-y-24">
          {filteredMenu.map((section) => (
            <div key={section.id}>
              <div className="flex items-center gap-6 mb-12">
                <h2 className="text-2xl font-serif font-bold text-white whitespace-nowrap">{section.title}</h2>
                <div className="h-px w-full bg-gradient-to-r from-white/10 to-transparent" />
              </div>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {section.items.map((item) => (
                  <DishCard key={item.name} item={item} onClick={(i) => setSelectedItem(i)} />
                ))}
              </div>
            </div>
          ))}
          ...
          {filteredMenu.length === 0 && (
            <div className="text-center py-24">
              <p className="text-white/30 text-lg">No dishes found matching your search.</p>
            </div>
          )}
        </div>
      </div>
